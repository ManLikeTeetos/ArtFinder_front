...
function UpdateUser() {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            const response = await fetch(`https://api.arthpathfinder.com/api/updateUser?session_username=${session_username}`, {
                method: 'POST',
                body: formData,
            });
    ...
    useEffect(() => {
        const fetchUserData = async () => {
           // alert(username);
            try {
                const response = await fetch(`https://api.arthpathfinder.com/api/getUser_upd?username=${session_username}`);
                if (response.ok) {
